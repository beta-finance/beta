from brownie import (
    a, interface, BetaBank, BToken, BetaConfig, BetaOracleUniswapV2, BetaInterestModelV1,
    BetaRunnerUniswapV2, MockExternalOracle, ERC20Contract, MockExternalOracle, WETHGateway,
    BTokenDeployer,
)
...
def main():
    ...
    rt = a[0].deploy(BetaRunnerUniswapV2, betaBank, weth, factory,
    ...
    router.addLiquidity(utoken, ctoken, mathval(40), mathval(40), 0, 0, a[9], 1700000000, {'from': a[9]})
    router.addLiquidityETH(utoken, mathval(40), 0, 0, a[9], 1700000000, {'value': mathval(40), 'from': a[9]})
    router.addLiquidityETH(ctoken, mathval(40), 0, 0, a[9], 1700000000, {'value': mathval(40), 'from': a[9]})
    ...
    rt.close(0, 2**256-1, mathval(10), [ctoken, utoken], 2**256-1,  {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 0))
    # collateral - weth, underlying - utoken -> weth
    rt.short(2**256-1, mathval(2), mathval(10), [utoken, weth], 0, {'value': mathval(10), 'from': a[1]})
    print(a[1].balance())
    ...
    rt.short(2**256-1, mathval(2), mathval(10), [utoken, weth, ctoken], 0, {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 3))
    rt.close(3, 2**256-1, mathval(10), [ctoken, weth, utoken], 2**256-1,  {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 3))
