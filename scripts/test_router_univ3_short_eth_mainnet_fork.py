from brownie import (
    a, chain, interface, BetaBank, BToken, BetaConfig, BetaOracleUniswapV2, BetaInterestModelV1,
    MockExternalOracle, ERC20Contract, MockExternalOracle, WETHGateway, BTokenDeployer,
    BetaRunnerUniswapV3,
)
...
def main():
    ...
    rt2 = a[0].deploy(BetaRunnerUniswapV3, betaBank, weth, factory,
    ...
    pools = [(utoken, ctoken), (utoken, weth), (ctoken, weth), (dtoken, weth), (dtoken, ctoken)]
    ...
    rt2.short([2**256-1, mathval(2), mathval(10), encode_path(path1, fee1), 0], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 0))
    rt2.close([0, 2**256-1, mathval(10), encode_path(path1, fee1), mathval(100)], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 0))

    # collateral - weth.address, underlying - utoken -> weth
    path2, fee2 = [utoken.address, weth.address], [FEE]
    rt2.short([2**256-1, mathval(2), mathval(10), encode_path(path2, fee2), 0], {'value': mathval(10), 'from': a[1]})
    print(a[1].balance())
    print(betaBank.positions(a[1], 1))
    rt2.close([1, 2**256-1, mathval(10), encode_path(path2, fee2), mathval(10)],  {'from': a[1]})
    print(a[1].balance())
    print(betaBank.positions(a[1], 1))

    # collateral - ctoken, underlying - weth.address -> ctoken
    path3, fee3 = [weth.address, ctoken.address], [FEE]
    rt2.short([2**256-1, mathval(2), mathval(10), encode_path(path3, fee3), 0], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 2))
    rt2.close([2, 2**256-1, mathval(10), encode_path(path3, fee3), mathval(100)], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 2))

    # collateral - ctoken, underlying - utoken -> weth.address -> ctoken
    path4, fee4 = [utoken.address, weth.address, ctoken.address], [FEE, FEE]
    rt2.short([2**256-1, mathval(2), mathval(10), encode_path(path4, fee4), 0], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 3))
    rt2.close([3, 2**256-1, mathval(10), encode_path(path4, fee4), mathval(100)], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 3))

    # multi-path, hit for loop, collateral - ctoken, underlying - utoken -> utoken -> weth -> dtoken -> ctoken
    path5, fee5 = [utoken.address, weth.address, dtoken.address, ctoken.address], [FEE, FEE, FEE]
    rt2.short([2**256-1, mathval(2), mathval(10), encode_path(path5, fee5), 0], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 4))
    rt2.close([4, 2**256-1, mathval(10), encode_path(path5, fee5), mathval(100)], {'from': a[1]})
    print(ctoken.balanceOf(a[1]))
    print(betaBank.positions(a[1], 4))
