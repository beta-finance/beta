...
from brownie import (
    ...
    ERC20Contract, MockExternalOracle, MockAllowActionForTester, BTokenDeployer,
...
def test_betabank_governor():
    ...
    betaBank.initialize(a[0], a[0].deploy(BTokenDeployer), ZERO, config, im)
...
def test_betabank_allow_action_self():
    ...
    im = a[0].deploy(BetaInterestModelV1, mathval(0.20), mathval(0.05), mathval(100), mathval(0.5))
    betaBank.initialize(a[0], a[0].deploy(BTokenDeployer), a[1], config, im)
    tester = a[0].deploy(MockAllowActionForTester, betaBank)
    betaBank.setOwnerWhitelists([a[1]], True, {'from': a[0]})
    assert tester.checkAllowActionFor(a[1], a[1], {'from': a[1]}).return_value
    assert tester.checkAllowActionFor(a[1], a[1], {'from': a[0]}).return_value
    assert not tester.checkAllowActionFor(a[1], a[0], {'from': a[1]}).return_value
    assert not tester.checkAllowActionFor(a[1], a[0], {'from': a[0]}).return_value
...
def test_betabank_allow_action_global():
    ...
    betaBank.initialize(a[0], a[0].deploy(BTokenDeployer), a[1], config, im)
    tester = a[0].deploy(MockAllowActionForTester, betaBank)
    assert not tester.checkAllowActionFor(a[1], a[5], {'from': a[1]}).return_value
    betaBank.setRunnerWhitelists([a[5]], True, {'from': a[0]})
    assert not tester.checkAllowActionFor(a[1], a[5], {'from': a[0]}).return_value
    assert tester.checkAllowActionFor(a[1], a[5], {'from': a[1]}).return_value
...
def test_betabank_public_create():
    ...
    betaBank.create(utoken2, {'from': a[0]})
...
def test_betabank_basic_borrow_repay():
    ...
    betaBank.setOwnerWhitelists([a[1]], True, {'from': a[0]})
    btoken = BToken.at(betaBank.bTokens(utoken))
    config.setCollFactors([ctoken], [mathval(0.8)], {'from': a[0]})  # 80% collateral factor
    ...
    assert betaBank.fetchPositionLTV(a[1], pid).return_value == mathval(0.15)  # (20*3) / (500*1*0.8)
    ...
    betaBank.repay(a[1], pid, mathval(5), {'from': a[1]})
    assert betaBank.fetchPositionLTV(a[1], pid).return_value == mathval(0.1125)  # (15*3) / (500*1*0.8)
    assert betaBank.positions(a[1], pid) == (28, 34, btoken,  ctoken, mathval(500), mathval(15))
...
def test_betabank_interest_liquidate():
    ...
    ext = a[0].deploy(MockExternalOracle)
    oracle = a[0].deploy(BetaOracleUniswapV2, WETH, ZERO, 3600)
    im = a[0].deploy(BetaInterestModelV1, mathval(0.2), mathval(0), mathval(100), mathval(0))
    utoken = a[0].deploy(ERC20Contract, 'My Underlying Token Name', 'UMYSYM')
    utoken.mint(a[0], mathval(1000000))
    ctoken = a[0].deploy(ERC20Contract, 'My Collateral Token Name', 'CMYSYM')
    ctoken.mint(a[1], mathval(1000000))
    ...
    assert float(betaBank.fetchPositionLTV(a[1], pid).return_value) == pytest.approx(
        mathval(0.30))  # (40*3) / (500*1*0.8)
    assert betaBank.positions(a[1], pid) == (25, 0, btoken,  ctoken, mathval(500), mathval(40))
    chain.sleep(365 * 86400)
    assert float(betaBank.fetchPositionLTV(a[1], pid).return_value) == pytest.approx(mathval(0.36))  # 20% interest
    chain.sleep(365 * 86400)
    assert float(betaBank.fetchPositionLTV(a[1], pid).return_value) == pytest.approx(mathval(0.432))  # 20% interest
    with brownie.reverts('liquidate/not-liquidatable'):
        betaBank.liquidate(a[1], pid, mathval(10), {'from': a[0]})
    chain.sleep(365 * 86400)
    assert float(betaBank.fetchPositionLTV(a[1], pid).return_value) == pytest.approx(mathval(0.5184))  # 20% interest
    assert betaBank.positions(a[1], pid) == (25, 0, btoken,  ctoken, mathval(500), mathval(40))
    ...
    assert float(betaBank.fetchPositionLTV(a[1], pid).return_value) == pytest.approx(mathval(0.361775), rel=1e-3)
...
def test_betabank_pausable():
    ...
    assert float(betaBank.fetchPositionLTV(a[1], pid).return_value) == pytest.approx(mathval(0.50625))
...
def test_betabank_altruistic_liquidate():
    ...
    with brownie.reverts('liquidate/not-liquidatable'):
        betaBank.liquidate(a[1], pid, mathval(10), {'from': a[0]})
    with brownie.reverts('emergencyLiquidate/positive-collateral'):
        betaBank.emergencyLiquidate(a[1], pid, mathval(40), {'from': a[0]})
    ...
    betaBank.emergencyLiquidate(a[1], pid, mathval(50), {'from': a[0]})
    [_, _, _, _, coll, debt] = betaBank.positions(a[1], pid)
    assert float(coll) == 0
    assert float(debt) == 0
