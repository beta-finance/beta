...
def test_btoken_basic_mint_burn():
    ...
    assert btoken.totalLoanable() == 999999999999999000000
...
def test_btoken_borrow_repay():
    ...
    config = a[0].deploy(BetaConfig, a[0], 0)
    im = a[0].deploy(
        BetaInterestModelV1, mathval(0.20), mathval(0), mathval(100), mathval(0)
    )  # 20% per year
    token = a[0].deploy(ERC20Contract, "My Token Name", "MYSYM")
    token.mint(a[0], mathval(1000000))
    ext = a[0].deploy(MockExternalOracle)
    ...
    assert btoken.totalLoanable() == 899999999999999000000
    ...
    assert float(btoken.totalLoanable()) == pytest.approx(899999999999999000000)
    ...
    assert float(
        btoken.fetchDebtShareValue(100, {"from": a[0]}).return_value
    ) == pytest.approx(110, abs=1)
    btoken.burn(a[3], mathval(200), {"from": a[1]})
    assert float(btoken.totalLoanable()) == pytest.approx(697999999999998980000)
    ...
    assert float(
        btoken.repay(a[0], mathval(50), {"from": betaBank}).return_value
    ) == pytest.approx(45454545454545454545)
    assert float(btoken.totalLoanable()) == pytest.approx(747999999999998980000)
...
def test_btoken_interest_rebase():
    ...
    config = a[0].deploy(BetaConfig, a[0], 0)
    im = a[0].deploy(
        BetaInterestModelV1, mathval(0.20), mathval(0), mathval(100), mathval(0.5)
    )  # 20% per year
    token = a[0].deploy(ERC20Contract, "My Token Name", "MYSYM")
    token.mint(a[0], mathval(1000000))
    ext = a[0].deploy(MockExternalOracle)
...
def test_btoken_accrue_interest():
    ...
    config = a[0].deploy(BetaConfig, a[0], 0)
    im = a[0].deploy(
        BetaInterestModelV1, mathval(0.10), mathval(0), mathval(100), mathval(0.5)
    )  # 10% per year
    token = a[0].deploy(ERC20Contract, "My Token Name", "MYSYM")
    token.mint(a[0], mathval(1000000))
    ext = a[0].deploy(MockExternalOracle)
